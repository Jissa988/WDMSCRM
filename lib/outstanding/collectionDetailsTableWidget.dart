...
class CollectionProductDataTable extends StatelessWidget {
  final List<CollectionItemDetails> collectionItemDetails;

  CollectionProductDataTable({
    required this.collectionItemDetails,
  });

  @override
  Widget build(BuildContext context) {
    double totalAmount = collectionItemDetails.fold(0, (previousValue, product) => previousValue + product.settled);
            ...
            rows: collectionItemDetails.map((item) {
              return DataRow(
                cells: [
                  ...
                  DataCell(
                    Text(
                    ' ${(item.billAmount-item.settled).roundOffStrict()}' ,
                      style: TextStyle(
                        fontSize: 15,
                        fontFamily: 'Metropolis',
                        color: Colors.black,
