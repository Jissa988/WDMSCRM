...
class _WaterDrop extends StatefulWidget {
  ...
  const _WaterDrop(
      {Key? key, required this.child, required this.top, required this.width, required this.left, required this.height})
      : super(key: key);
    ...
    Widget childWithGradient = Container(
      foregroundDecoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [AppColors.waterDrops, AppColors.white],
        ),
...
class OvalClipper extends CustomClipper<Path> {
  ...
  OvalClipper({required this.center, required this.width, required this.height});
...
class _LightDot extends StatelessWidget {
  ...
  const _LightDot({Key? key, required this.top, required this.left, required this.width, required this.height})
      : super(key: key);
...
class _OvalShadow extends StatelessWidget {
  ...
  const _OvalShadow({Key? key, required this.top, required this.left, required this.width, required this.height})
      : super(key: key);
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              blurRadius: 4,
              offset: Offset(4, 4),
              color: AppColors.waterDrops.withOpacity(0.5),
            ),
          ],
        ),
      ),
